...
public class JsonParser {
	...
	public JsonObject parse(final String jsonStr) {
		...
		do {
			r = parse(obj, str);
		} while (r == JsonParseResult.STILL_WORKING);
		
		if (r == JsonParseResult.SUCCEEDED) {
			return obj;
		}
	...
	private JsonParseResult parse(final JsonObject obj, final JsonString json) {
		if (mLastSyntax == JsonSyntax.END) {
			return JsonParseResult.SUCCEEDED;
		}
		
		if (json.end()) {
			mLogger.log(TAG, "Malformed JSON; end found before closing brace");
			return JsonParseResult.FAILED;
		}
		...
		prepareJson(json);
		return JsonParseResult.STILL_WORKING;
	}
	...
	private boolean parseValue(final JsonString json, final JsonValue value) {
		...
		if (curChar == '{') {
			...
			if (firstFour.contentEquals("null")) {
				value.setNull();
